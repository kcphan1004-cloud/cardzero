...
export default function AccountMenu() {
  ...
  if (loading) {
    return <div className="h-10 w-24 animate-pulse rounded-xl bg-zinc-900" aria-label="正在读取登入状态" />;
  }
      ...
      {open && (
        <div role="menu" className="absolute right-0 top-[calc(100%+10px)] z-[120] w-64 overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-950 shadow-[0_24px_80px_rgba(0,0,0,.75)]">
          <div className="border-b border-zinc-900 px-4 py-4">
            ...
            <p className="mt-2 truncate text-sm font-bold text-white">{email}</p>
            <p className="mt-1 text-xs text-emerald-400">● 云端同步已启用</p>
          </div>

          <div className="p-2">
            <Link href="/deck-builder" onClick={() => setOpen(false)} className="block rounded-xl px-3 py-3 text-sm font-bold text-zinc-300 transition hover:bg-zinc-900 hover:text-white">
              我的卡组
            </Link>

...
function getRarityValue(rarity: string) {
...
function hasTranslation(card: Card) {
  return Boolean(
    card.nameZh ||
      card.effectZh ||
      card.triggerZh,
...
const [gridMode, setGridMode] =
  ...
  const translatedCardCount = useMemo(
    () =>
      cards.filter((card) =>
        hasTranslation(card),
      ).length,
